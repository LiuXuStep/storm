 ...
 * dingli
 * 
 * $("#fs").lqfieldset({ title:'个人信息', //标题显示文字 collapsible:true, //是否可伸缩
 * collapsed:false, //初始化状态 是否为收起状态 checkboxToggle:false, //伸缩按钮是否为checkbox
 * onBeforeCollapse:function(){ //收起之前调用该方法，返回false则取消收起 },
 * onCollapse:function(){ //收起之后调用该方法 }, onBeforeExpand:function(){
 * //展开之前调用该方法，返回false则取消展开 }, onExpand:function(){ //展开之后调用该方法 } });
 * 
 * $("#fs").lqfieldset('collapse'); //收起组件 $("#fs").lqfieldset('expand'); //展开组件
 */
;
(function($) {
	// 添加自定义方法
	...
	function initDom(thisObj) {
		...
		var innerDom = $(thisObj)
				.wrap("<fieldset class='lq-fieldset'></fieldset>");
		...
		if (thisOptions.title || thisOptions.collapsible) {
			...
			if (checkboxToggle) {
				innerDom = innerDom
						.before("<legend class='lq-fieldset-legend'>"
								+ '<input type="checkbox" />' + '<span>'
								+ thisOptions.title + "</span></legend>");
			...
			if (!thisOptions.collapsible) {
				...
				fieldSetDom.find('.lq-fieldset-legend input[type=checkbox]')
						.remove();
			}
		}
